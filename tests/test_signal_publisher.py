...
def test_signal_format_is_safe_for_public_posting():
    signal = Signal("BTCUSDT", SignalSide.BUY, 0.8, datetime.now(timezone.utc), ["EMA bullish"])
...
def test_square_publisher_is_disabled_without_supported_api():
    with pytest.raises(NotImplementedError):
        BinanceSquarePublisher().publish(Signal("BTCUSDT", SignalSide.HOLD, 0, datetime.now(timezone.utc)))
...
def test_duplicate_signal_is_published_once():
    ...
    signal = Signal("BTCUSDT", SignalSide.BUY, 0.8, datetime.now(timezone.utc))
